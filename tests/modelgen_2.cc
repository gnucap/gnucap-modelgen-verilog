...
public:
  void do_it(CS& cmd, CARD_LIST*)override {
    ...
    trace1("mg_2", cmd.tail());

    std::string name;
    cmd >> name;

    std::string module_content;
    cmd.get_line("verilog-modelgen>");
    while (cmd >> "discipline"){
      module_content += cmd.fullstring();
      for (;;) {
	cmd.get_line("verilog-disc>");
	module_content += cmd.fullstring();
    ...
    while (cmd >> "primitive"){ untested();
      module_content += cmd.fullstring();
      for (;;) { untested();
	cmd.get_line("verilog-primitive>");

	module_content += cmd.fullstring();
	if (cmd >> "endprimitive ") { untested();
